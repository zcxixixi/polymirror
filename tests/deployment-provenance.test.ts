...
describe("Candidate deployment provenance", () => {
  ...
  it("keeps the shadow cohort isolated, preview-validated, and rollback-safe", () => {
    ...
    expect(compose).toContain("SHADOW_EXPECTED_ACCOUNT_COUNT=${SHADOW_EXPECTED_ACCOUNT_COUNT:-6}");
    expect(compose).toContain("health.enabledAccountCount !== expectedAccountCount");
    expect(compose).toContain("health.polledAccountCount !== expectedAccountCount");
    expect(compose).toContain("health.experiments.length !== expectedAccountCount");
    ...
    expect(deploy).toContain("approved cohort roster must contain 1 to 10 Candidates");
    expect(deploy).toContain("duplicate approved Candidate identity");
    expect(deploy).not.toContain("quality6-20260711-v1");
    expect(deploy).not.toContain("0xb55fa1296e6ec55d0ce53d93b9237389f11764d4");
    expect(deploy).not.toContain("0xcc500cbcc8b7cf5bd21975ebbea34f21b5644c82");
    ...
    expect(deploy).toContain("loaded.accounts.length !== expectedAccountCount");
    expect(deploy).toContain("buildCandidateExperimentConfig");
    expect(deploy).toContain("mounted config does not match project-native Candidate builder output");
    ...
    expect(deploy).toContain("health.enabledAccountCount !== expectedAccountCount");
    expect(deploy).toContain("health.polledAccountCount !== expectedAccountCount");
    expect(deploy).toContain('logs --no-color --tail 200');
    expect(deploy).toMatch(/grep -q .*accountCount.*SHADOW_EXPECTED_ACCOUNT_COUNT/);
